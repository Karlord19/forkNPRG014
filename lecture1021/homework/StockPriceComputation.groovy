...
group.with {
    ...
    def avgPricesCopy = new DataflowQueue()
    ...
    operator(inputs: [parisEURPrices, viennaEURPrices, frankfurtEURPrices, chicagoEURPrices], outputs: [avgPrices, avgPricesCopy], stateObject: [parisPrice: 0]) { paris, vienna, frankfurt, chicago ->
        if (paris != 0) {
        ...
        def res = (paris + vienna + frankfurt + chicago) / 4
        bindAllOutputs res
    }

    // Operator 3: Calculate a five-day moving average
    operator(inputs: [avgPricesCopy], outputs: [fiveDayAverages], stateObject: [history: []]) { avg ->
        stateObject.history << avg
        if (stateObject.history.size() > 5) {
            stateObject.history.remove(0)
        }
        int res = stateObject.history.sum() / stateObject.history.size()
        bindOutput res
    }
